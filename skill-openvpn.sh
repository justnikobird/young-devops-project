...
if ! grep -rhEq "^deb https://repo.justnikobird.ru:1111/lab focal main" /etc/apt/sources.list.d/own_repo.list 
then
...
if $2 == nat
then
    if ! iptables -t nat -C $1 &> /dev/null
    then
        iptables -t nat -A $1
    fi
fi
...
while true; do
    ...
    case $cs in
        [Cc]* ) 
                apt-get update
                if command_check openvpn "Openvpn" just-open-vpn
                then
                ...
                while true; do
                    ...
                    case $proto in
                        tcp) 
                                sed -r -i 's/^proto\sudp$/\;proto udp/' /etc/openvpn/server/server.conf
                                sed -r -i 's/^\;proto\stcp$/proto tcp/' /etc/openvpn/server/server.conf
                                sed -r -i 's/^proto\sudp$/\;proto udp/' /etc/openvpn/clients_config/confiles/base.conf
                                sed -r -i 's/^\;proto\stcp$/proto tcp/' /etc/openvpn/clients_config/confiles/base.conf
                                sed -r -i 's/^explicit-exit-notify\s1$/\;explicit-exit-notify 1/' /etc/openvpn/server/server.conf; break;; 
                        udp)
                                break;;
...
while true; do
    ...
    case $cs in
        [Cc]* ) 
                ...
                if /etc/openvpn/clients_config/make_config.sh $client_crt_file $client_key_file $client_name
                then
