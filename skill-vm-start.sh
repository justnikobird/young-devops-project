...
while true; do
    read -n 1 -p "Continue or Skip? (c|s) " cs
    case $cs in
        [Cc]* ) 
                while true; do
...
while true; do
    ...
    case $cs in
        [Cc]* ) 
                ...
                read -p "password for repo.justnikobird.ru (default Sm1rn0V187187): " -s repo_pass
                
                if ! grep -Fxq "deb https://repo.justnikobird.ru:1111/lab focal main" /etc/apt/sources.list.d/own_repo.list &> /dev/null
                then echo "deb https://repo.justnikobird.ru:1111/lab focal main" >> /etc/apt/sources.list.d/own_repo.list 
                fi
                
                if ! grep -Fxq "machine repo.justnikobird.ru:1111" /etc/apt/auth.conf &> /dev/null
                then echo -e "machine repo.justnikobird.ru:1111\nlogin $repo_login\npassword $repo_pass" >> /etc/apt/auth.conf
                ...
                if ! wget --no-check-certificate -P ~/ https://$repo_login:$repo_pass@repo.justnikobird.ru:1111/lab/labtest.asc 
                then
                    restore_bkp /etc/apt/sources.list.d/own_repo.list
                    restore_bkp /etc/apt/auth.conf
                    exit 1
                ...
                if ! wget --no-check-certificate -P /usr/local/share/ca-certificates/ https://$repo_login:$repo_pass@repo.justnikobird.ru:1111/lab/ca.crt
                then
                    restore_bkp /etc/apt/sources.list.d/own_repo.list
                    restore_bkp /etc/apt/auth.conf
                    exit 1
...
echo -e "\n====================\nIptables config\n===================="
while true; do
    read -n 1 -p "Current ssh session may drop! To continue you have to relogin to this host via 1870 ssh-port and run this script again. Are you ready? (y|n) " yn
    case $yn in
        [Yy]* ) #---DNS---
